...
Set_Token(){
	while true
	do
		echo -e "请输入Server酱调用代码 [SCKEY]
Token，通过Server酱获取。
获取步骤：Server酱会告诉一个调用代码，例如：https://sc.ftqq.com/abc，其中的 abc 就是SCKEY。"
...
POST_TG(){
	Get_IP
	if [[ -z "${NAME}" ]]; then
		wget -qO- --post-data="text=QAQ VPS娘貌似坏掉啦!  —  ${IP}&desp=主人，VPS娘貌似坏掉啦~ IP地址：${IP}&parse_mode=Markdown&disable_notification=false"  "https://sc.ftqq.com/${TOKEN}.send" >> ${LOG_file}
	else
		wget -qO- --post-data="text=QAQ VPS娘貌似坏掉啦!  —  [${NAME}] ${IP}&desp=主人，VPS娘貌似坏掉啦~ VPS名称：${NAME} IP地址：${IP}&parse_mode=Markdown&disable_notification=false"  "https://sc.ftqq.com/${TOKEN}.send" >> ${LOG_file}
	fi
	echo "" >> ${LOG_file}
}
Get_IP(){
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/zhuleizi/storehouse/master/gfw_push.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	wget -N --no-check-certificate "https://raw.githubusercontent.com/zhuleizi/storehouse/master/gfw_push.sh" && chmod +x gfw_push.sh
